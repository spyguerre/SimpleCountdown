Loop {
    Loop, 8 {
        ...
        if (ErrorLevel = 0) {
            ImageSearch, _, _, 0, 0, A_ScreenWidth, A_ScreenHeight, *20 CPpng/ValidateCP%A_Index%.png
            if (ErrorLevel = 0) {
                ...
                ImageSearch, _, _, 0, 0, A_ScreenWidth, A_ScreenHeight, *20 CPpng/ValidateAltCP%A_Index%.png
                if (ErrorLevel = 0) {
                    FileRead, WhichCPContent, WhichCP.txt
                    if ((A_Index != 1) && (WhichCPContent != A_Index)) {
                        SoundPlay, SFX/check.wav
